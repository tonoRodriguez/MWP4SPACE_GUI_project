...
def SSC_sim(length,angle_side,wg_max,wg_min,cell_points, wl, # basic tapper simulation
    ...
    width_l=wg_max
    width_r=wg_min
    delta_w = 2*(height1 + height2)*np.tan((90-angle_side)*np.pi/180);
    ...
    zmin = -(height2)/2; #altura maxima
    zmax = (height2)/2; #altura minima
    ...
    ymax_bot_r = width_bot_r/2; #ancho max en el lado derecho abajo
    ...
    mode1.addrect(name="Si_core1",x_min = xmin- length/2 -centered_x , x_max=xmax,
    		 y = centered_y, y_span=width_film_y, z_min = centered_z,
    ...
    mode1.addrect(name="SiO2_cladding2",x_min = xmin- length/2 -centered_x , x_max=xmax,
    		 y = centered_y, y_span=width_film_y, z_min = centered_z,
    ...
    mode1.addplanarsolid(name="SSC",x = centered_x,y = centered_y, z= centered_z+ height2/2, material = "Si_Salzberg")
    mode1.set('vertices',vtx)
    mode1.set('facets',b)
    
    mode1.addplanarsolid(name="Input",x = centered_x ,y = centered_y, z= centered_z+ height2/2, material = "Si_Salzberg")
    mode1.set('vertices',vtx_input)
    mode1.set('facets',b)
    

    
    mode1.addmesh(name = "mesh_waveguide", x_min = xmin , x_max = xmax,
      		y = centered_y , y_span= width_bot_l, z = centered_z + height1/2, z_span = (height1 + height2),
      		override_x_mesh = 0, override_y_mesh = 1,
      		override_z_mesh = 1, set_maximum_mesh_step = 1,
      		dy = 15e-9, dz = 15e-9)
    
    mode1.addeme(x_min = -length, y = centered_y, y_span= width_bot_l,z=centered_z, z_span= 0.3e-6, wavelength = wl)
...
def mmi_sim(ssc_length,angle_side,wg_max,wg_min,cell_points,MMi_width,MMi_length,distance,wl, # basic tapper simulation
    ...
    mode1.addrect(name="Si_core1",x = centered_x, x_span = 2*length + MMi_length,
    		 y = centered_y, y_span=width_bot_r_MMi*1.5, z_min = centered_z,
    		 z_max= centered_z+ height1, material = "Si_Salzberg"  )
    ...
    mode1.addrect(name="SiO2_cladding2",x = centered_x, x_span = 2*length + MMi_length,
    		 y = centered_y, y_span=width_bot_r_MMi*1.5, z_min = centered_z,
    		 z_max= centered_z +base_hight*0.25, material = "SiO2_fusedquartz" , alpha = 0.5 )
    ...
    mode1.addplanarsolid(name="SSC_l",x = centered_x - MMi_length/2 -length/2,y = centered_y, z= centered_z + height2/2, material = "Si_Salzberg")
...
#S = SSC_sim(5e-6,80,1e-6,0.5e-6,19,1,2e-6)
