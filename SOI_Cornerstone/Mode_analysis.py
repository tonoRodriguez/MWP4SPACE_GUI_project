...
def mode_analysis(waveguide_w,angle,boundry,rad,wl):
    ...
    mode1.addrect(name = "SiO2 Clad" ,x = centered_x, x_span=width_film_x,
    		 y = centered_x, y_span=width_film_y, z_min = centered_z, z_max= base_hight + centered_z + 1e-6, material = "SiO2_fusedquartz" ,alpha = 0.3)
    centered_z=centered_z + base_hight;
    
    mode1.addwaveguide(name = "Si", base_angle = angle, base_height= LN_hight_sub, base_width=waveguide_w, material = "Si_Salzberg")
    mode1.setnamed("Si","poles",np.array([[-width_film_y/2,centered_y],
                                                 [width_film_y/2,centered_y]]))
    mode1.setnamed("Si","z",centered_z)
    

    
    mode1.addmesh(name = "mesh_waveguide", x_min = centered_x - 0.1e-6, x_max = centered_x+ 0.1e-6,
    		y = 0 , y_span= waveguide_w+0.5e-6, z = centered_z, z_span = 1e-6,
...
#mode_analysis(0.5e-6, 90,"PML",0,1.5e-6)
